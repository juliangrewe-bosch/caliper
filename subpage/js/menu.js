...
function menuItem(item, level, parent, group) {
  if (parent) {
    var dataParent =
      ' data-parent="' + (level == 0 ? "ROOT" : "menu-" + parent) + '"';
  } else {
    var dataParent = "";
  }
  ...
  if (group)
    ...
    var expandButton =
      ...
      '" style="margin-left: ' +
      level * 10 +
      'px;" class="expand-button hidden">&nbsp;</span>';
...
function setActiveMenu() {
  $(".nav a").each(function () {
    const navA = $(this);
    if (
      !navA.hasClass("expand-button") &&
      navA.attr("href") == getLink(window.location.pathname)
    ) {
